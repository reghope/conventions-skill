...
6. Propose the complete change in human-readable form before writing it, unless the user explicitly asked to apply the curation. If there is a conflict between established patterns, stop and ask the user which direction is authoritative.
7. After approval, update only the managed block and its curation metadata: refresh the `Health` line, and set `Maturity`: keep `young` while rules are few or lightly corroborated, and raise it to `established` once the core rules have held across several tasks or been confirmed by the user. Do not rewrite product code to make the policy look consistent, and do not touch user content outside the block.
8. Whenever a change is applied, end the response with a brief report:
