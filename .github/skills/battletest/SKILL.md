...
> smolt users: this skill is implemented natively as the `/battletest` extension — a per-tester `browse` tool (private headless browser, screenshot per action), storage under `.smolt/battletest/<run>/`, a live roster whose lines say what each tester is doing right now (self-narrated actions, with a cheap-model classifier as fallback), expandable per-tester ticket and action lists in the desktop, filing-time dedupe with an `append` action, per-action metrics, enforced budgets, performance records in `performance.json`/`form.jsonl`, and the clearance loop wired through the `battletest` tool's `wait`/`decide` actions. Testers run as in-memory temporary sessions on the session's own model at low thinking.
